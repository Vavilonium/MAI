...
try:
    ...
    for i in range(k_number):
        for j in range(k_number):
            if i == j:
                ...
                matrex[i][j] = int(input('Введите данные попарного сравнения критериев ' + str(j + 1) + " и " + str(i + 1)
                                         + " : "))
    ...
    for i in range(k_number):  # Вывод весовых коэффициентов для всех критериев
        print("Весовой коэффициент для " + str(i + 1) + "-го критерия: " + str(root[i] / root_summ))
except ValueError:
